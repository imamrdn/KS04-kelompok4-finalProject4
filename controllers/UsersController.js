...
class UsersController {
  static async Register(req, res, next) {
    ...
    try {
      ...
      const validateUsername = await Models.User.findOne({
      ...
      const validatePhoneNumber = await Models.User.findOne({
  ...
  static async Login(req, res, next) {
    try {
      ...
      const validateEmail = await Models.User.findOne({
        where: {email: email},
        attributes: ["id", "password"],
      });
      if (!validateEmail) return res.status(404).json("Email Not Registered");
      if (!compare(password, validateEmail.password))
  ...
  static async Update(req, res, next) {
    ...
    try {
      if (id != usersId)
        return res.status(400).json({message: "Tidak punya akses"});
